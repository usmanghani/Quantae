...
var reciteList = {
    ...
    'fooladvand': {
        name: 'Persian: فولادوند',
        base: 'Fooladvand_Hedayatfar_40Kbps',
        trans: 'fa.fooladvand'
    },
    'makarem': {
        name: 'Persian: مکارم شیرازی',
        base: 'Makarem_Kabiri_16Kbps',
        trans: 'fa.makarem'
    }
};
var defRecite = 'afasy';
...
var Scroller = {
    ...
    scrollToAya: function () {
        ...
        if (currTab == 'trans') obj = $('#t-' + curr.sura + '-' + curr.aya);
        if (obj.length == 0) return;
        if (curr.aya == 1) obj = obj.prevUntil('span').last();
        var top = obj.offset().top;
        var bottom = top + obj.height();
        if (currTab == 'quran' && opt.showTrans == 'fixed') bottom = Math.max(bottom, $('.tbox').offset().top + $('.tbox').height());
...
function addBookmark(title, url) {
    ...
    else if (isIE && window.external) window.external.AddFavorite(url, title);
    else {
...
var History = {
    ...
    load: function (args) {
        ...
        var url = '';
        if (args.tab == 'trans') url = 'trans/' + (args.id) + '/';
        if (args.tab == 'search') url = 'search/' + args.type + '/' + args.query + (args.page == 1 ? '' : '/p' + args.page);
        else url += args.sura + ':' + args.aya;
        $.history.load(url);
    },
    show: function (hash) {
        var self = History;
        if (hash == '') {
            if (self.callState == 'new') self.load();
            ...
            try {
                ...
                var searchArgs = {};
                var type = args[0];
                if (type != 'trans' && type != 'search') type = 'quran';
                currTab = type;
                if (type == 'search') {
                    searchArgs = {
                        type: args[1],
    ...
    locate: function (addr) {
        var res = {};
        var p = addr.split('-');
        if (p[0] == 'page' || p[0] == 'juz') res = Quran[p[0] + 'Props'](1 * p[1]);
        else {
            var q = addr.split(':');
            res = {
...
function initPage(start) {
    ...
    Player.init();
    if (!localhost) setTimeout('loadGoogleAnalytics()', 2000);
...
function initUI() {
    ...
    $('#gotoText').val(curr.sura + ':' + curr.aya);
    $('#customFont').val(opt.customFont);
    $('#fontSize').val(opt.fontSize);
...
function initAd() {
    var t = opt.ad;
    if (t & 1) return;
    setTimeout("$('.top-ad').fadeIn()", 2000);
    setTimeout("update('opt.ad', opt.ad | 1)", 5 * 60 * 1000);
    $('.top-ad .tbox-close').click(function () {
        $('.top-ad').fadeOut();
        update('opt.ad', t | 1);
    });
}
...
function adjustDefaults(start) {
    if (start.defTrans) Storage.data.curr.trans = start.defTrans;
    if (isMSafari) Storage.data.opt.showTrans = 'none';
}
...
function initMenu(menu, items, defVal) {
    ...
    for (var i = 0; i < items.length; i++) {
        var opt = '';
        if (/^sep-/.test(i)) opt += ' disabled="disabled"';
...
function changeSura(offset) {
    ...
    if (offset < 0 && curr.aya > 1) offset = 0;
...
function refresh() {
    currPage = '';
    if (currTab == 'search') Search.display();
    else display();
}
...
function display(sura, aya) {
    ...
    if (page + ':' + curr.quran + ':' + curr.trans + ':' + currTab == currPage) updatePage(page);
...
function setAudioTrans() {
    ...
    if (reciteList[opt.reciters[i].id].trans == curr.trans) active = true;
...
function updateAudioTransSettings() {
    ...
    if (active) {
        for (var i in opt.reciters)
        if (reciteList[opt.reciters[i].id].trans == curr.trans) removeReciter(i);
    } else {
        for (var i in reciteList)
        if (reciteList[i].trans == curr.trans) addReciter(i);
    }
    setAudioTrans();
}
...
var Search = {
    ...
    goTo: function () {
        ...
        addr = addr.split(':');
        displayAya(addr[0], addr[1] || 1);
    },
    showPage: function (offset) {
        var page = 1 * $('#pageOffset').val() + offset;
        page = Math.max(page, 1);
        page = Math.min(page, $('#totalPages').text());
        $('#pageOffset').val(page);
        this.page = page;
        History.load({
            tab: 'search',
    ...
    draw: function (results, numAyaMatch, totalMatch) {
        if (results.length == 0) this.page = 0;
        $('#pageOffset').val(this.page);
...
function initPageActions() {
    ...
    $('.aya').hover(function () {
        ...
        if (!$(this).hasClass('selected')) {
            var busy = $(this).children().first().css('backgroundColor') != inherit;
            if (!busy && !$(this).hasClass('hover')) return;
            $(this).children().stop().css(busy ? {} : {
                backgroundColor: hoverColor
            }).animate({
                backgroundColor: orgColor
    ...
    if (currTab == 'quran' && opt.showTrans == 'hover') initTransTip($('.aya'));
}
...
function addReciter(reciterID) {
    if (opt.reciters.length >= 4) return;
    opt.reciters.push({
...
Storage = {
    ...
    save: function (name) {
        for (var i in this.data)
        if (!name || i == name) this.write(i, window[i]);
    },
    reset: function (name) {
        for (var i in this.data)
        if (!name || i == name) window[i] = $.clone(this.data[i]);
        this.save();
    },
    upgrade: function () {
        ...
        if (opt.version == '1.4.3') curr.quran = {
            'simple-plain': 'simple'
        }[curr.quran] || curr.quran;
        opt.version = version;
    },
    ...
    write: function (name, object) {
        ...
        if (this.medium == 'localStorage') localStorage[name] = val;
...
function updateFontSettings() {
    update('opt.fontSize', $('#fontSize').val() * 1);
...
TextTools.isValidReg = function (pattern) {
    try {
...
TextTools.fixText = function (text, args) {
    args = args || {};
    if (isFF4) text = this.pregReplace('($WAW$REH$END$HARAKA)', '$1$ZWNJ', text);
    if (args.showSigns) {
...
TextTools.addZWNJ = function (text) {
...
var Dialog = {
    ...
    setButtons: function () {
        ...
        for (var button in buttons) {
            ...
            $('<span/>').addClass('button').attr('id', id).text(button).click(buttons[button]).disableTextSelect().appendTo($('.dbox-footer'));
        }
    },
...
var Player = {
    ...
    currItem: {},
    delayID: null,
    hasBism: ['abdulbasit-mjwd', 'ajamy', 'parhizgar', 'fooladvand', 'makarem'],
    init: function () {
        var self = this;
        ...
        if (this.inspector && isTest) $.getScript(root + '/js/jquery/jplayer/jquery.jplayer.inspector.js', function () {
    ...
    loadAya: function () {
        if (!this.ready) return;
        if (this.loadedAya == curr.sura + ':' + curr.aya) return;
    ...
    load: function () {
        ...
        if (this.buffer && this.preloadEnabled) this.buffer.jPlayer('clearMedia');
    ...
    preload: function (item) {
        if (!this.buffer || !this.preloadEnabled || opt.playScope == 'aya') return;
        var certificate = item.sura + ':' + item.aya + ':' + item.reciter
        if (this.preloadedAya != certificate) this.buffer.jPlayer('setMedia', {
    ...
    resetPlayList: function () {
        ...
        for (var i in opt.reciters) {
            var reciter = opt.reciters[i];
            if (i == 0 && curr.aya == 1 && curr.sura != 1 && curr.sura != 9 && !this.bismIncluded(reciter.id)) this.playList.push({
                sura: curr.sura,
                aya: 0,
                reciter: reciter.id
            });
